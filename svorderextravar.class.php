...
class SvorderExtraVar
{
	...
	/**
	 * Current module's Set of SvorderExtraItem
	 * @var SvorderExtraItem[]
	 */
	var $keys = null;

	/**
	 * Get instance of SvorderExtraVar (singleton)
	 *
	 ...
	 * @return void
	 */
	function SvorderExtraVar($module_srl)
	{
	...
	function setExtraVarKeys($extra_keys)
	{
		...
		{
			$obj = new SvorderExtraItem($val->module_srl, $val->idx, $val->name, $val->type, $val->default, $val->desc, $val->is_required, $val->search, $val->value, $val->eid);
			$this->keys[$val->idx] = $obj;
		}
	}

	/**
	 * Returns an array of SvorderExtraItem
	 *
	 * @return SvorderExtraItem[]
	 */
...
class SvorderExtraItem
{
	 ...
	 * @return void
	 */
	function SvorderExtraItem($module_srl, $idx, $name, $type = 'text', $default = null, $desc = '', $is_required = 'N', $search = 'N', $value = null, $eid = '')
	{
		if(!$idx)
		{
			return;
	...
	function getFormHTML()
	{
		...
		switch($type)
		{
			...
			// textarea
			case 'textarea' :
				$buff[] = '<textarea name="' . $column_name . '">' . $value . '</textarea>';
				break;
			// multiple choice
			case 'checkbox' :
			...
			case 'radio' :
				foreach($default as $v)
				{
					...
					// Temporary ID for labeling
					$tmp_id = $column_name . '-' . $id_num++;
					$buff[] = '<input type="radio" name="' . $column_name . '" id="' . $tmp_id . '" ' . $checked . ' value="' . $v . '"  class="radio" /><label for="' . $tmp_id . '">' . $v . '</label>';
				}
				break;
			// date
			case 'date' :
...
/* End of file SvorderExtraVar.class.php */
/* Location: ./modules/svorder/SvorderExtraVar.class.php */